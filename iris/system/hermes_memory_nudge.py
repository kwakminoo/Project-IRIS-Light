...
_MARKER = "<!-- iris-control-nudge-v3 -->"
_BLOCK = """<!-- iris-control-nudge-v3 -->
## Iris Light UI control
When the user asks to open IDE / start coding / Companion / "ide 켜줘" / open a project / 아이리스 라이트 작업 시작:
1. Prefer MCP tools `iris_get_state`, `iris_get_catalog`, `iris_invoke` (skills: iris-work-start, iris-work-end, iris-session-status).
...
def ensure_memory_nudge() -> str:
    ...
    if _MARKER in existing and "ide.enter_companion" in existing and "ide 켜줘" in existing:
    ...
    return "memory nudge updated (v3)"
